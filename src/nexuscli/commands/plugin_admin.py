...
def _load_translations(language: str) -> Callable[[str], str]:
...
def _parse_plugin_language_overrides(
    values: list[str],
    translate: Callable[[str], str] | None = None,
) -> dict[str, str]:
    """Parse repeated plugin language values in format plugin_id:lang."""
    _ = translate or (lambda message: message)
    ...
    for raw in values:
        ...
        if ":" not in entry:
            raise typer.BadParameter(
                _("set_plugin_language must use 'plugin_id:lang' format")
            )
        ...
        if not plugin_id:
            raise typer.BadParameter(
                _("set_plugin_language requires a non-empty plugin id")
            )

        language_code = _normalize_language_code(language_raw)
        if language_code is None:
            raise typer.BadParameter(
                _("language must be ISO-639-1 format, e.g., en, zh, ja")
            )
...
def _print_changes(
    ...
    table = Table(title=translate("Config Changes"))
    table.add_column(translate("Field"))
    table.add_column(translate("Current"))
    table.add_column(translate("New"))
...
def _print_current_settings(
    ...
    table = Table(title=translate("Current Plugin Settings"))
    table.add_column(translate("Field"))
    table.add_column(translate("Value"))

    table.add_row("entrypoint_group", settings.entrypoint_group)
    table.add_row("conflict_policy", settings.conflict_policy)
    table.add_row(
        "enable_folder_loader",
        _format_setting_value(settings.enable_folder_loader),
    )
...
def _build_app(translate: Callable[[str], str]) -> typer.Typer:
    _ = translate
    app = typer.Typer(help=_("Manage plugin lifecycle and runtime status."))

    @app.command(
        "list",
        help=_("List loaded plugins and diagnostics for this invocation."),
    )
    def list_plugins(
        show_failed: bool = typer.Option(
            True,
            "--failed/--no-failed",
            help=_("Show failed plugins."),
        ),
        show_skipped: bool = typer.Option(
            True,
            "--skipped/--no-skipped",
            help=_("Show skipped plugins."),
        ),
    ) -> None:
        ...
        if state.loaded_plugins:
            table = Table(title=_("Loaded Plugins"))
            table.add_column(_("Plugin ID"))
            table.add_column(_("Source"))
            table.add_column(_("Command Path"))
            ...
            console.print(
                Panel.fit(
                    _("No plugin loaded in current invocation."),
                    title=_("Loaded Plugins"),
                    border_style="yellow",
                )
            )

        if show_skipped and state.skipped_plugins:
            table = Table(title=_("Skipped Plugins"))
            table.add_column(_("Plugin ID"))
            table.add_column(_("Reason"))
        ...
        if show_failed and state.failed_plugins:
            table = Table(title=_("Failed Plugins"))
            table.add_column(_("Plugin"))
            table.add_column(_("Error"))
    ...
    @app.command(
        "disable",
        help=_("Disable a plugin for next CLI invocations."),
    )
    def disable_plugin(
        plugin_id: str = typer.Argument(..., help=_("Plugin ID to disable.")),
    ) -> None:
        ...
        if changed:
            console.print(
                Text(
                    _("Disabled plugin '{plugin_id}'. Effective next invocation.").format(
                        plugin_id=plugin_id,
                    ),
                    style="green",
                )
            ...
            console.print(
                Text(
                    _("Plugin '{plugin_id}' was already disabled.").format(
                        plugin_id=plugin_id,
                    ),
                    style="yellow",
                )
    ...
    @app.command(
        "enable",
        help=_("Enable a previously disabled plugin."),
    )
    def enable_plugin(
        plugin_id: str = typer.Argument(..., help=_("Plugin ID to enable.")),
    ) -> None:
        ...
        if changed:
            console.print(
                Text(
                    _("Enabled plugin '{plugin_id}'. Effective next invocation.").format(
                        plugin_id=plugin_id,
                    ),
                    style="green",
                )
            ...
            console.print(
                Text(
                    _("Plugin '{plugin_id}' was already enabled.").format(
                        plugin_id=plugin_id,
                    ),
                    style="yellow",
                )
    ...
    @app.command(
        "inspect",
        help=_("Inspect one plugin from current runtime state."),
    )
    def inspect_plugin(
        plugin_id: str = typer.Argument(..., help=_("Plugin ID to inspect.")),
    ) -> None:
        ...
        if rec is not None:
            table = Table(title=_("Plugin Details"))
            table.add_column(_("Field"))
            table.add_column(_("Value"))
            table.add_row("plugin_id", rec.plugin_id)
            table.add_row("source", rec.source)
            table.add_row("command_path", rec.command_path)
            table.add_row("version", rec.version)
            table.add_row("help", rec.help_text)
            console.print(table)
            return
        ...
        if plugin_id in state.disabled_plugins:
            console.print(
                Panel.fit(
                    _("Plugin '{plugin_id}' is currently disabled.").format(
                        plugin_id=plugin_id,
                    )
                    + "\n"
                    + _("Config file: {path}").format(path=get_config_path()),
                    title=_("Disabled Plugin"),
                    border_style="yellow",
                )
            )
            return
        ...
        if reason:
            console.print(
                Panel.fit(
                    _("Plugin '{plugin_id}' is not active. Reason: {reason}").format(
                        plugin_id=plugin_id,
                        reason=reason,
                    ),
                    title=_("Inactive Plugin"),
                    border_style="red",
                )
            )
            return
        ...
        console.print(
            Panel.fit(
                _("Plugin '{plugin_id}' not found in current invocation.").format(
                    plugin_id=plugin_id,
                ),
                title=_("Plugin Not Found"),
                border_style="red",
            )
        )
    ...
    @app.command(
        "config",
        help=_("Update plugin-related configuration with preview and validation."),
    )
    def update_config(
        entrypoint_group: str | None = typer.Option(
            None,
            "--entrypoint-group",
            help=_("Set plugin discovery entry point group."),
        ),
        conflict_policy: str | None = typer.Option(
            None,
            "--conflict-policy",
            help=_("Set command conflict policy: error or skip."),
        ),
        enable_folder_loader: bool | None = typer.Option(
            None,
            "--folder-loader/--no-folder-loader",
            help=_("Enable or disable folder-based plugin discovery."),
        ),
        set_plugin_dir: list[str] = typer.Option(
            [],
            "--set-plugin-dir",
            help=_("Replace plugin directory list. Repeat option for multiple values."),
        ),
        add_plugin_dir: list[str] = typer.Option(
            [],
            "--add-plugin-dir",
            help=_("Add one plugin directory. Repeat option for multiple values."),
        ),
        remove_plugin_dir: list[str] = typer.Option(
            [],
            "--remove-plugin-dir",
            help=_("Remove one plugin directory. Repeat option for multiple values."),
        ),
        language: str | None = typer.Option(
            None,
            "--language",
            help=_("Set global language code (ISO-639-1, e.g., en, zh, ja)."),
        ),
        set_plugin_language: list[str] = typer.Option(
            [],
            "--set-plugin-language",
            help=_("Set plugin language as 'plugin_id:lang'. Repeat option for multiple values."),
        ),
        remove_plugin_language: list[str] = typer.Option(
            [],
            "--remove-plugin-language",
            help=_("Remove plugin language override by plugin id. Repeat option for multiple values."),
        ),
        clear_plugin_languages: bool = typer.Option(
            False,
            "--clear-plugin-languages",
            help=_("Clear all plugin language overrides."),
        ),
        dry_run: bool = typer.Option(
            False,
            "--dry-run",
            help=_("Preview changes without writing the config file."),
        ),
        yes: bool = typer.Option(
            False,
            "--yes",
            "-y",
            help=_("Apply changes without confirmation prompt."),
        ),
    ) -> None:
        ...
        if not has_direct_options:
            _print_current_settings(current, translate=_)
            console.print(Text(_("No option provided. Starting interactive setup."), style="bold"))
            ...
            conflict_policy = typer.prompt(
                _("Conflict policy (error/skip)"),
                default=current.conflict_policy,
            )
            language = typer.prompt(
                _("Global language (ISO-639-1)"),
                default=current.language,
            )
            enable_folder_loader = typer.confirm(
                _("Enable folder loader"),
                default=current.enable_folder_loader,
            )
            plugin_dirs_text = typer.prompt(
                _("Plugin directories (comma-separated)"),
                default=", ".join(current.plugin_dirs),
            )
            set_plugin_dir = [
                item.strip() for item in plugin_dirs_text.split(",") if item.strip()
            ]
            plugin_languages_text = typer.prompt(
                _("Plugin language overrides (plugin_id:lang, comma-separated)"),
                default=", ".join(
                    f"{plugin_id}:{lang}"
        ...
        if entrypoint_group is not None:
            ...
            if not cleaned_group:
                raise typer.BadParameter(_("entrypoint_group cannot be empty"))
        ...
        if conflict_policy is not None:
            ...
            if cleaned_policy not in VALID_CONFLICT_POLICIES:
                ...
                raise typer.BadParameter(
                    _("conflict_policy must be one of: {allowed}").format(
                        allowed=allowed,
                    )
                )
        ...
        if remove_plugin_dir:
            ...
            proposed.plugin_dirs = [
                directory
                for directory in proposed.plugin_dirs
        ...
        if set_plugin_language:
            language_updates = _parse_plugin_language_overrides(
                set_plugin_language,
                translate=_,
            )
        ...
        if remove_plugin_language:
            for plugin_id in remove_plugin_language:
                ...
                if not cleaned_plugin_id:
                    raise typer.BadParameter(
                        _("remove_plugin_language cannot contain empty plugin id")
                    )
                proposed.plugin_languages.pop(cleaned_plugin_id, None)

        if not proposed.plugin_dirs:
            raise typer.BadParameter(_("plugin_dirs cannot be empty"))

        changes = _settings_changes(current, proposed)
        if not changes:
            console.print(Text(_("No config changes detected."), style="yellow"))
            return

        _print_changes(changes, translate=_)

        if dry_run:
            console.print(
                Text(
                    _("Dry-run mode. No file written. Config path: {path}").format(
        ...
        if not yes and not typer.confirm(_("Apply these changes?"), default=True):
            console.print(Text(_("Update canceled."), style="yellow"))
            return

        save_settings(proposed)
        console.print(
            Text(
                _("Config updated: {path}").format(path=get_config_path()),
                style="green",
            )
        )
        _print_current_settings(proposed, translate=_)

    @app.command(
        "config-path",
        help=_("Show where plugin config is stored."),
    )
...
class PluginAdminPlugin(CliPluginBase):
    ...
    def __init__(self) -> None:
        ...
        self.plugin_app: typer.Typer | None = None
    ...
    def metadata(self) -> CommandMetadata:
        return CommandMetadata(
            ...
            command_name="plugin",
            help_text=self._("Plugin management commands"),
            version="0.1.0",
    ...
    def typer_app(self) -> typer.Typer:
        if self.plugin_app is None:
            self.plugin_app = self.build_app()
        return self.plugin_app

    def on_configure(self, context: dict[str, Any]) -> None:
        language = _resolve_language(context.get("language"))
        self.language = language
        self._ = _load_translations(language)

    def build_app(self) -> typer.Typer:
        self.plugin_app = _build_app(self._)
        return self.plugin_app
